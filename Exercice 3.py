...
depression_filename = 'Epidemiology of depression - Wikipedia.mhtml'
sunshine_filename = 'List of cities by sunshine duration - Wikipedia.mhtml'
...
#Parcourt les lignes du tableau HTML.
#Récupère le nom du pays et le taux de DALY (taux de dépression).
#Retourne un DataFrame pandas avec les pays comme index.
...
#Dictionnaire pour stocker les heures d’ensoleillement par pays.
#Les indices correspondent aux colonnes du tableau.
...
#Parcourt tous les tableaux et lignes.
#Récupère les heures annuelles d’ensoleillement, les convertit en heures mensuelles.
#Si plusieurs villes pour un pays → prend la moyenne.
#Retourne un DataFrame pandas avec les pays comme index.
    ...
    for table in sunshine_tables:
        ...
        #On ne regarde pas le taux d'ensolleilement par mois => On prend le total annuel que l'on divise par 12.
...
#Fusionne les deux DataFrames sur les pays communs.
...
#Calcule la corrélation de Pearson entre la dépression et l’ensoleillement.
#Affiche un scatterplot avec seaborn.
#Le titre indique la corrélation.

#On crée un CSV pour sauvegarder les données fusionnées.
